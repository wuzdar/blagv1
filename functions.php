...
function insert($data = array()) {
  ...
  // Prepare and bind
  $stmt = $conn->prepare("INSERT INTO forma (name, oib, interests, address, city, state) VALUES (?, ?, ?, ?, ?, ?)");
  $stmt->bind_param("ssssss", $name, $oib, $interests, $address, $city, $state);
...
function show_results($insert_id) {
  ...
  // Prepare and bind
  $stmt = $conn->prepare("SELECT name, oib, interests, address, city, state FROM forme WHERE ID = ?");
